...
module Engine
  ...
  def initialize(cmd_line_mode=true, interactive_mode=true, colourised=true)
    ...
    if cmd_line_mode
      ...
      while true do
        if $turn_counter == 1
            handle_line("random_move")
        else
          if line = Readline.readline('> ', true)  
            handle_line(line)
          end  
        end
      end
    ...
    s = line.split(" ")
      if s[0] =~ /^exit$/i
        ...
        if s.size==2 and @games.size<=@@MAX_GAMES
          ...
          g = Game.new(2, @games.size.to_s, @interactive_mode, @colourised)
          puts "size 1"
...
#engine = Engine::CeltiaRandomEngine.new(true)
